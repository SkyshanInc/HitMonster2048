...
res_list = {}
res_dir = ''
if os.path.exists(os.path.join(os.path.dirname(__file__), '../pokoSanguo/Resources/')):
    res_dir = os.path.join(os.path.dirname(__file__), '../pokoSanguo/Resources/')
else:
...
for root, dirs, files in os.walk(res_dir):
    for f in files:
        ...
        if f not in ignores:
            ...
            res_list[res_path] = [md5_for_file(open(res_abs_path)),os.path.getsize(res_abs_path)];
        else:
            print 'ignore file: %s' % f;
print res_list;
#write to resource list file
res_list_file = open(os.path.join(res_dir, "resource_internal_test.lst"), 'w')
